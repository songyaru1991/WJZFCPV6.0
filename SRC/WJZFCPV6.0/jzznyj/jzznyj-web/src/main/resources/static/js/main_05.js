$(function () {
  ...
  function resize1 () {
    ...
    $('.A105_directory').height($('.A105L_body').height() - 320); // 173
...
//点击收起表格
  ...
  // jquery  点击显示弹窗*/
  $('.A105P_close,.A105_addBtn,.A105P_zhenju,.A105P_zhenju1 ').click(function () {
...
//选项卡初始化
  ...
  $('.H105_clickEvent').click(function () {
    ...
    console.log('重新加载gdy3' + layHref);
    parent.parent.$('.src_iframeleft').attr('src', layHref);

  });

  // // 添加标签 所处三级页面gdy
  $('.rgznjz').click(function () {
    var ajbs = $(this).attr('ajbs');
    var ajmc = $(this).attr('ajmc');
    var type = $(this).attr('type');
    var layContent = $(this).attr('data-content');
    var layHref = $(this).attr('lay-href');
    layHref = layHref+'?ajbs='+ajbs+'&ajmc='+ajmc+'&type='+type
    // 新建或者替换标签
    addTab(layContent, layHref);
    console.log('重新加载gdy33' + layHref);
    parent.parent.$('.src_iframeleft').attr('src', layHref);
  });
  ...
  $('.H105_labelCoat').on('click', '.H105_label', function () {
    ...
    var layHref = $(this).attr('lay-href');
    // 重新加载
    console.log('重新加载gdy1' + layHref);
    if('案件列表'== layContent){
      $('.src_iframeleft').attr('src', '/jzznyj/znjz/navigationBarCaseList');
    }else if('律师授权'== layContent) {
      $('.src_iframeleft').attr('src', '/jzznyj/znjz/navigationBarLssq');
    }else if('人工智能阅卷'== layContent) {
      $('.src_iframeleft').attr('src', '/jzznyj/znyj/navigationBarZnyj');
    }else{
  ...
  function addTab (content, layHref) {
    parent.parent.$('.H105_labelCoat>.H105_label').removeClass('H105_label2');
    // 判断标签个数
    if ( parent.parent.$('.H105_label').length > 1) {
      parent.parent.$('.H105_labelCoat>.H105_label:eq(1)').replaceWith(' <div class="H105_label H105_label2 fl" lay-href=' + layHref + ' data-content=' + content + '><span>' + content + '</span><img src="/jzznyj/static/images/H105_x1.png" alt="" class="H105_close"></div>');
    } else {
      parent.parent.$('.H105_labelCoat').append(' <div class="H105_label H105_label2 fl" lay-href=' + layHref + ' data-content=' + content + '><span>' + content + '</span><img src="/jzznyj/static/images/H105_x1.png" alt="" class="H105_close"></div>');
    }
  }
  ...
  $('.H105_labelCoat').on('click', '.H105_close', function (event) {
    ...
    if ($('.H105_label2').index() == 2) {
      ...
      layContent= $('.H105_labelCoat>.H105_label:eq(0)').attr('data-content');
      // 重新加载
      console.log('重新加载gdy delete' + layHref);
      if('案件列表'== layContent){
        $('.src_iframeleft').attr('src', '/jzznyj/znjz/navigationBarCaseList');
      }else if('律师授权'== layContent) {
        $('.src_iframeleft').attr('src', '/jzznyj/znjz/navigationBarLssq');
  ...
  $('.jq_suspect').click(function () {
    ...
    console.log('重新加载gdy edit' + layHref);
    parent.parent.$('.src_iframeleft').attr('src', layHref);
